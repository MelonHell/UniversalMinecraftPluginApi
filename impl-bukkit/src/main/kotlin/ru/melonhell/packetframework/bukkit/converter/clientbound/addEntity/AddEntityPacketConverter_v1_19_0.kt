package ru.melonhell.packetframework.bukkit.converter.clientbound.addEntity
...
import ru.melonhell.packetframework.core.protocol.game.clientbound.CbAddEntityPacketWrapper
...
//        }
    ...
    override fun wrap(container: PacketContainer): CbAddEntityPacketWrapper {
        ...
        val entityType = EntityType.fromMinecraftName(bukkitEntityType.key.toString()) ?: throw RuntimeException("unknown entity type")
        // Yaw Pitch HeadYaw
        val rotation = Look(
            container.bytes.read(1) * 360.0f / 256.0f,
            container.bytes.read(0) * 360.0f / 256.0f
    ...
    override fun unwrap(wrapper: PacketWrapper): List<PacketContainer> {
        ...
        if (wrapper.entityType == EntityType.EXPERIENCE_ORB) {
        ...
        if (wrapper.entityType == EntityType.PLAYER) {
